			...
			jQuery(document).ready(function(){
				jQuery("select[id=card_machine_indicated]").change(function() {
					...
					switch(jQuery("#card_machine_indicated option:selected").val()) {
						case "money":
							jQuery("p.woo_payment_indicated").append("<input id='woo_cash_delivery' class='input-text' type='number' style='padding:10px;' maxlength='20' placeholder='0,00' pattern='-?\d+(,\d{2})?' step='1.00' name='woo_cash_delivery'/><span><?php echo __('*Enter 0 to inform you that do not need to change.', 'woo-payment-on-delivery'); ?></span><hr/><span><?php echo $this->notice_money; ?></span>");
							break;
						case "paycheck":
							...
							break;
						case "debitcard":
							jQuery("p.woo_payment_indicated").append("<select style='padding:10px;' id='flagcard_indicated' name='flagcard_indicated'><option value=''><?php echo $select_flag; ?></option><option value='Mastercard'>Mastercard</option><option value='Visa'>Visa</option><option value='Cabal'>Cabal</option><option value='Sicredi'>Sicredi</option></select><?php if(!empty($this->notice_card)) { ?><hr/><span><?php echo $this->notice_card; ?></span><?php } ?>");
							break;
						case "creditcard":
